...
这一段没有官方代码片段。transcript 明确给出的 key API 是 untracked resource、Metal fence 和 event：当同一资源被相邻 pass 复用，但数据无关时，可以关闭资源跟踪，再由应用插入同步。下面用最小片段表达这个用法边界。

```swift
// Transcript-supported synchronization sketch
let scratchBuffer = device.makeBuffer(
    length: scratchLength,
    options: [.storageModePrivate, .hazardTrackingModeUntracked]
)!
let fence = device.makeFence()
renderEncoder.updateFence(fence, after: .fragment)
nextRenderEncoder.waitForFence(fence, before: .vertex)
```

关键点：

- `.hazardTrackingModeUntracked` 对应 transcript 中的 “marked as untracked”，让 Metal 不再为 `scratchBuffer` 自动建立依赖。
- `device.makeFence()` 对应 transcript 提到的 Metal fences，用来把同步责任交回应用。
- `updateFence(... after: .fragment)` 表示前一个 pass 的 fragment 写入完成后更新 fence。
- `waitForFence(... before: .vertex)` 表示后一个 pass 的 vertex 读取前等待 fence，避免真正的数据依赖被误删。
...
（[36:03](https://developer.apple.com/videos/play/wwdc2020/10632/?time=2163)）要让 compiler 更容易把 constant buffer preload 到 uniform register，应把常量组织成单个 struct，通过 `constant` address space 传引用；数组大小最好在编译期已知。
...
fragment half4 shade(
    constant LightingConstants& constants [[buffer(0)]],
    uint lightIndex
) {
    LightInfo light = constants.lights[lightIndex];
    return half4(light.color, 1.0h);
}
```

关键点：

- `MAX_LIGHTS` 让数组大小在编译期可见，符合 session 对 constant array 的建议。
- `LightingConstants` 把常量数据封装成单个 struct。
- `constant LightingConstants&` 使用 constant address space，并按引用传入。
- `constants.lights[lightIndex]` 访问固定大小数组中的 light 数据。
- `1.0h` 是 half literal，避免把 half 表达式提升到 32-bit float。
