...
主な設計: ユーザーの IP アドレスとアクセス先のドメイン名の両方を知ることは誰にもできません。Apple はあなたが誰であるかを知っているだけで、あなたが何にアクセスしているかは知りません。 2 番目のエージェントは、あなたが何にアクセスしているかを知っているだけで、あなたが誰であるかは知りません。
...
- **怎么开始**：在现有 URLSession 回调中加入 `task.metrics` 检查，记录 `isProxyConnection`ログに記録するステータス
