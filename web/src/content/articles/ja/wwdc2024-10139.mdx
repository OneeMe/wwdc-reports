...
デフォルトでは、Vision Pro の画面キャプチャは app ウィンドウのみで背景は除去。此 API を有効化すると、キャプチャ内容に Passthrough カメラビュー全体— ユーザーが見る完全な空間シーンが含まれます。
...
以前 visionOS では Core ML モデルは CPU と GPU のみで実行可能。此 API が Apple Neural Engine を解放し、より高い演算力を必要とする ML モデルをデバイスローカルで実行可能に。
