...
relatedSessions:
  - title: "Explore the Action & Vision app"
    code: "10099"
    description: "Create ML のアクション分類と Vision の姿勢・軌跡・輪郭検出を組み合わせ、運動分析サンプル App を分解します。"
  - title: "Detect Body and Hand Pose with Vision"
    code: "10653"
    description: "Vision が写真や動画内で身体と手の姿勢を検出し、アクション分類に姿勢入力を提供する方法を説明します。"
  - title: "Control training in Create ML with Swift"
    code: "10156"
    description: "Swift で Create ML のトレーニングフローを制御し、進捗を確認してモデル学習を自動化する方法を示します。"
---
## ハイライト
...
([03:45](https://developer.apple.com/videos/play/wwdc2020/10043/?time=225)) トレーニング データの基本的な形式はビデオ ファイルのセットです。各ビデオにはアクションが 1 つだけ含まれており、フォルダー名はモデルによって予測されるラベルです。セッションの例では、ジャンピング ジャックやスクワットなど 5 種類のスポーツを収集し、2 つのネガティブ カテゴリも追加します。 `other` そして `none`。

([05:28](https://developer.apple.com/videos/play/wwdc2020/10043/?time=328)) モンタージュ ビデオに複数のアクションが含まれている場合は、CSV または JSON を使用して各アクションの開始時刻と終了時刻をマークします。公式コード スニペットは、JSON 構造を示します。
...
([14:05](https://developer.apple.com/videos/play/wwdc2020/10043/?time=845)) モデルは Vision によって抽出された人間のポーズを受け取り、最初に元のビデオからポーズ抽出が行われます。最小エントリーポイントは `VNDetectHumanBodyPoseRequest`:
...
([14:10](https://developer.apple.com/videos/play/wwdc2020/10043/?time=850)) ビデオ ファイルを処理するときに、このリクエストを `VNVideoProcessor`:
...
([14:26](https://developer.apple.com/videos/play/wwdc2020/10043/?time=866)) 写真またはカメラ フレームを処理する場合、セッションは `VNImageRequestHandler`:
...
- への入力 `VNImageRequestHandler` 画像 URL または `CVPixelBuffer`、 `CMSampleBuffer`、 `CGImage`、 `CIImage`。
- `perform([request])` 姿勢検出を実行します。
- その結果も `[VNRecognizedPointsObservation]`。

### 連続ポーズをモデル入力に結合する

([14:57](https://developer.apple.com/videos/play/wwdc2020/10043/?time=897)) Create ML によってエクスポートされたアクション分類モデルは、3 次元のマルチ配列を受け取ります。各フレームの姿勢はまず次のように変換されます。 `[1, 3, 18]`、60 個のフレームが組み立てられます。 `[60, 3, 18]`:
...
([16:27](https://developer.apple.com/videos/play/wwdc2020/10043/?time=987)) Xcode デモでは、ポーズ抽出、ウィンドウ キャッシュ、予測がカプセル化されています。 `Predictor`:
...
- **ミックスカットビデオ注釈ガジェットを作成**: ユーザーがマークできるようにします `file_name`、 `label`、 `start_time` そして `end_time` 長いビデオを作成し、ML トレーニングの作成用に JSON または CSV にエクスポートします。これにより、最初に各アクションを個別のビデオにカットすることなく、既存の映像を再利用できます。
