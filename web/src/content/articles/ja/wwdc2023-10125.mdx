...
> これは、Mac へのゲーム移植の 3 部作の最終章です。レンダラーを Metal に移植する方法について説明しており、リソースのバインディングと永続性、コマンド送信の最適化、間接レンダリング、および MetalFX 超解像度という 4 つの主要なトピックを取り上げています。
...
**重要なポイント**:
- `MTLResourceID` は Metal 3 で導入されたリソース識別子です。
- テクスチャ テーブルはレンダリング ループではなく、初期化フェーズ中に作成されます。
- `gpuResourceID` はテクスチャの GPU 側識別子を直接取得します
...
**重要なポイント**:
- `supportArgumentBuffers = YES` は、サンプラーを引数バッファーに入れるための前提条件です。
...
- `gpuAddress` サブテーブルの GPU アドレスを取得します
- レンダリング ループでこの最上位バッファをバインドするだけで済みます: `[encoder setVertexBuffer:topAB offset:0 atIndex:0]`
...
**読み取り専用リソース**: 大きなヒープにグループ化され、エンコーダー `useHeap` ごとに 1 回呼び出されます。
...
- エンコーダーごとに `useHeap` 呼び出しは 1 つだけ

([06:49](https://developer.apple.com/videos/play/wwdc2023/10125/?time=409))

**書き込み可能なリソース**: 個別に割り当て、`useResource` で使用フラグを指定し、Metal に同期を処理させます。
...
- `useResource` を使用して読み取りおよび書き込みの意図を明示的に指定します
...
シーンに何千もの描画コマンドがあり、CPU エンコード時間がボトルネックになっている場合は、ICB を使用してください。 ICB は、パイプライン状態やバッファ バインディングの設定などのコマンドを GPU 上でエンコードします。
...
- **やること**: 引数バッファを使用して既存のルート署名バインディング モデルを移植します。
  - **䡡値**: Metal 3 の引数バッファーはパフォーマンスが高く、Direct3D の記述子テーブルの概念を完全にマッピングできます。
  - **Hajime Kata**: 初期化フェーズ中にテクスチャ テーブルとサンプラー テーブルをエンコードし、レンダリング ループはトップレベルの引数バッファのみをバインドします。

- **やること**: Metal Debugger の Insights 機能を使用して、帯域幅最適化の機会を自動的に発見します。
  - **䡡値**: どのパスをマージできるか、どのストア操作が冗長であるかを直感的に確認できます。
  - **KataHajime**: Xcode で Metal ワークロードをキャプチャし、概要ビューアの Insights セクションを表示します。

- **やること**: CPU エンコーディングのボトルネック シナリオで間接コマンド バッファーを使用する
  - **䡡値**: コマンド エンコーディングを CPU から GPU に移動し、CPU オーバーヘッドを大幅に削減します。
  - **Haigata**: 間接描画パラメータを ICB コマンドに変換し、`executeCommandsInBuffer` で実行する計算カーネルを追加します。

- **やること**: 統合された MetalFX アップスケーリングによりフレーム レートが向上します
  - **䡡値**: 低解像度でレンダリングし、オーバーサンプリングして Apple Silicon のパフォーマンスを大幅に向上させます。
  - **ハジメ**: エンジンが超解像度と手動 LOD 制御をサポートしていることを確認し、空間アルゴリズムまたは時間アルゴリズムを選択します
