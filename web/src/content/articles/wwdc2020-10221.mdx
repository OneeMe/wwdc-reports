...
这场 session 的重点不是追求一个抽象的“快”。它先保证测试不会无限卡住，再用 spindump 指向挂起位置，最后把适合拆分的测试并行分发。这样，CI 报告从“没有结果”变成“有失败、有诊断、有总耗时”，团队才能根据结果行动。
...
- `executionTimeAllowance` 是 `XCTestCase` 上的 `TimeInterval` property。
- 返回值用于给这个测试类设置更细粒度的运行时间上限。
...
xcodebuild test \
  ...
  -destination 'platform=iOS,id=<device-2>' \
...
- `xcodebuild test` 从命令行启动测试，适合 CI。
- 多个 `-destination` 指定可参与分发的设备或模拟器。
- `-parallel-testing-enabled YES` 打开并行测试能力。
- `-parallelize-tests-among-destinations YES` 要求 xcodebuild 把测试分散到这些 destination，而不是在每个 destination 上跑完整套测试。
