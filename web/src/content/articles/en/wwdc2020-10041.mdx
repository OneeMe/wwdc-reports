...
([05:10](https://developer.apple.com/videos/play/wwdc2020/10041/?time=310)) Document-based apps use `DocumentGroup`. transcript explains that it automatically manages document opening, editing, and saving scenarios and is available on iOS, iPadOS, and macOS.```swift
import SwiftUI
...
([05:49](https://developer.apple.com/videos/play/wwdc2020/10041/?time=349)) Document App can also add menu commands. `commands` will place the custom menu in the appropriate location for the platform, and `keyboardShortcut` will display the shortcut keys.```swift
DocumentGroup(newDocument: ShapeDocument()) { file in
    DocumentView(document: file.$document)
}
.commands {
...
([09:22](https://developer.apple.com/videos/play/wwdc2020/10041/?time=562)) List adds a new children key path, which can directly display recursive hierarchical data. This capability reduces frequent push-and-pop navigation in content-based apps.```swift
...
([10:09](https://developer.apple.com/videos/play/wwdc2020/10041/?time=609)) The grid uses lazy-loading layout. `GridItem(.adaptive(minimum: 176))` will adjust the number of columns to the available width.```swift
...
([10:58](https://developer.apple.com/videos/play/wwdc2020/10041/?time=658)) Custom long lists can be used with `LazyVStack`. The official gallery example also shows that the view builder supports `switch`, which can switch multiple layouts in the same stack.```swift
...
([12:24](https://developer.apple.com/videos/play/wwdc2020/10041/?time=744)) SwiftUI adds a new `toolbar` modifier. Toolbar items can be ordinary Views, which are placed in the platform's preferred location by default.```swift
struct ContentView: View {
    var body: some View {
        List {
            Text("Book List")
...
([12:40](https://developer.apple.com/videos/play/wwdc2020/10041/?time=760)) Use `ToolbarItem` when you need to control the position. It can declare semantic placements such as primary operations, confirm operations, cancel operations, and principal positions.```swift
struct ContentView: View {
    var body: some View {
        List {
            Text("Book List")
...
([15:28](https://developer.apple.com/videos/play/wwdc2020/10041/?time=928)) The `help` modifier becomes a tooltip on macOS and also provides an accessibility hint.```swift
struct ContentView: View {
    var body: some View {
        Button(action: recordProgress) {
...
([17:08](https://developer.apple.com/videos/play/wwdc2020/10041/?time=1028)) New controls cover progress and capacity expressions. `ProgressView` represents definite or uncertain progress, `Gauge` represents the position of a certain value relative to capacity.```swift
struct ContentView: View {
...
([19:17](https://developer.apple.com/videos/play/wwdc2020/10041/?time=1157)) `matchedGeometryEffect` connects the same element in two areas. In the example, when the album moves from the grid to the selected row, SwiftUI interpolates the frame to form a continuous transition.```swift
struct ContentView: View {
...
([20:34](https://developer.apple.com/videos/play/wwdc2020/10041/?time=1234)) Responsive typography has also been enhanced. Custom fonts will scale with Dynamic Type, and non-text metrics can use `@ScaledMetric`.```swift
struct ContentView: View {
    var album: Album
...
([23:15](https://developer.apple.com/videos/play/wwdc2020/10041/?time=1395)) The system integration part will first talk about `Link` and `openURL`. `Link` will open a URL or universal link, `openURL` is used to trigger the opening from code in the View environment.```swift
...
([25:16](https://developer.apple.com/videos/play/wwdc2020/10041/?time=1516)) Sign in with Apple also has SwiftUI controls. Just import both `AuthenticationServices` and `SwiftUI` to use it.```swift
