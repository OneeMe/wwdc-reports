...
- use`NEFilterProvider`and`NEFilterDataProvider` 实现过滤逻辑
...
- **做什么**：检查项目中所有使用 `http://`URLs, all upgraded to`https://`
- **Why it’s worth it**: Private Relay will proxy insecure HTTP traffic, but this is only intermediate protection; end-to-end encryption is the complete solution
...
- **How ​​to start**: Add to existing URLSession callback`task.metrics` 检查，记录 `isProxyConnection` 状态到日志
...
- [Network extension for the modern Mac](/articles/wwdc2021-10106) — 现代 Mac 网络扩展开发，包括 VPN 和代理场景
