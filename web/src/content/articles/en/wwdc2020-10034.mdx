...
relatedSessions:
  - title: "Meet WidgetKit"
    code: "10028"
    description: "A WidgetKit overview covering how SwiftUI, stateless experiences, and timelines keep widgets fresh."
  ...
  - title: "Widgets Code-along, part 2: Alternate timelines"
    code: "10035"
    description: "The second episode in the same code-along: continue with timelines, system intelligence, multiple sizes, and configuration."
  - title: "Widgets Code-along, part 3: Advancing timelines"
    code: "10036"
    description: "The third episode in the same code-along: move into advanced timelines, background URLs, deep links, and multiple widgets."
  - title: "Design great widgets"
    code: "10103"
    description: "Covers glanceable widget ideas, content, color, size, layout, and typography from a design perspective."
---
...
([02:39](https://developer.apple.com/videos/play/wwdc2020/10034/?time=159)) The first step in turning a view into a widget is to create a new Widget Extension target in Xcode. After the new target is activated, add the required view files to the target, and then use the widget size to check the effect in SwiftUI Preview.
...
([04:23](https://developer.apple.com/videos/play/wwdc2020/10034/?time=263)) `DisplayName` and `Description` need to be added to the Widget template, and then enter the main `EntryView`. `EmojiRangerWidgetEntryView` already has entry, character data can be placed in the entry and then passed to `AvatarView`.
...
([06:22](https://developer.apple.com/videos/play/wwdc2020/10034/?time=382)) After running for the first time, the Add sheet displays small, medium, and large at the same time. The presenter believes that it is not ready to support all sizes at this time, so it only opens one size that it can currently do well.
