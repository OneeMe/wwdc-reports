...
([04:34](https://developer.apple.com/videos/play/wwdc2020/10052/?time=274)) A new look for `UIPageControl` allows the control to support an arbitrary number of pages within a fixed size; users can scrub and scroll between page points when space is insufficient. It also adds indicator pictures and background style customization, which is suitable for pages with special meanings such as bookmark pages and collection pages.
...
([14:50](https://developer.apple.com/videos/play/wwdc2020/10052/?time=890)) Menus that have been opened can also be updated. `updateVisibleMenu` of `UIContextMenuInteraction` will pass in a copy of the current menu and return a new menu to replace the current display content.
...
([16:05](https://developer.apple.com/videos/play/wwdc2020/10052/?time=965)) `UIAction` was introduced in iOS 13 and expanded to more controls in iOS 14. `UIBarButtonItem` adds a new initializer, which can create buttons that only execute actions, buttons that only display menu, or buttons that support both tap action and long-press menu.
