...
- **Adaptive Media Quality**: In video playback applications, based on`isExpensive`and`isConstrained`Dynamically adjust video bitrate.5G unlimited package users automatically play 4K, standard mode plays 1080p, low data mode reduces to 720p or pauses automatic playback.Entrance API:`NWPathMonitor` + `AVPlayerItem`的 preferredPeakBitRate。
