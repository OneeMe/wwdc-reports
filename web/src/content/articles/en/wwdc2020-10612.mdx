...
（[06:53](https://developer.apple.com/videos/play/wwdc2020/10612/?time=413)）Scene Understanding 是 `ARView` 的 environment 相关设置。它包含四个选项：`occlusion`、`receivesLighting`、`physics`、`collision`。
...
（[20:10](https://developer.apple.com/videos/play/wwdc2020/10612/?time=1210)）渲染问题经常藏在资源里。法线方向不对、纹理坐标错位、roughness 或 metallic 参数异常，都可能让模型看起来不真实。RealityKit 新增 Debug Model Component，用指定调试属性覆盖实体显示。
...
（[21:57](https://developer.apple.com/videos/play/wwdc2020/10612/?time=1317)）Location Anchors 使用真实世界坐标创建锚点。因为 `ARGeoAnchor` 是 `ARAnchor` 的子类，RealityKit 可以用已有的 anchor initializer 创建 anchor entity。
...
- [Gain insights into your Metal app with Xcode 12](/articles/wwdc2020-10605) — 与 RealityKit 的 Debug Model Component 一起补齐图形问题定位工具链。
