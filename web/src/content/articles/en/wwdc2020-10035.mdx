...
relatedSessions:
  - title: "Widgets Code-along, part 1: The adventure begins"
    code: "10034"
    description: "The first episode in the same code-along: create the widget target, connect the provider, and ship systemSmall first."
  - title: "Widgets Code-along, part 3: Advancing timelines"
    code: "10036"
    description: "The third episode in the same code-along: continue with advanced timelines, URLs, deep links, and multiple widgets."
  - title: "Add configuration and intelligence to your widgets"
    code: "10194"
    description: "A deeper look at widget configuration, system intelligence, Siri suggestions, and Smart Stack capabilities."
  - title: "Design great widgets"
    code: "10103"
    description: "Covers widget ideas, content, color, size, layout, and typography from a design perspective."
  - title: "Build SwiftUI views for widgets"
    code: "10033"
    description: "Explains how to build widget views from scratch with SwiftUI and polish a glanceable interface."
---
...
([07:39](https://developer.apple.com/videos/play/wwdc2020/10035/?time=459)) Emoji Rangers characters regenerate over time. Now that the app knows the `endDate` of a full restore, the provider can generate entries at one-minute intervals until the character restore is complete.
...
([09:00](https://developer.apple.com/videos/play/wwdc2020/10035/?time=540)) The timeline entry also has an optional `relevance`. When the widget is in the stack, the system can rotate the widget to a more appropriate position based on this prompt.
...
([09:56](https://developer.apple.com/videos/play/wwdc2020/10035/?time=596)) After the timeline is completed, the widget still only displays Power Panda. The demo then uses the SiriKit Intent Definition file to add configuration items to allow users to select roles on the Home Screen.
...
([13:42](https://developer.apple.com/videos/play/wwdc2020/10035/?time=822)) Demonstrates adding the `widgetURL` modifier to the view after the configuration is completed. In this way, when the user clicks on the widget of the current role, he will directly enter the corresponding role details page.
