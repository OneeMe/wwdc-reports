...
（[06:58](https://developer.apple.com/videos/play/wwdc2020/10611/?time=418)）位置锚点从 `ARGeoTrackingConfiguration` 开始。Session 明确要求先检查两件事：设备是否支持 geo tracking，当前位置是否有足够的地图数据用于视觉定位。
...
（[08:38](https://developer.apple.com/videos/play/wwdc2020/10611/?time=518)）Session 用旧金山 Ferry Building 做示例。开发者创建 `CLLocationCoordinate2D`，再创建 `ARGeoAnchor`，最后把 RealityKit 的实体挂到这个锚点上。
...
（[10:32](https://developer.apple.com/videos/play/wwdc2020/10611/?time=632)）`ARGeoAnchor` 的坐标轴固定到方位：X 轴指向东，Z 轴指向南，Y 轴向上。锚点本身不可变，旋转和抬高内容要在 RealityKit 实体上完成。
...
（[14:08](https://developer.apple.com/videos/play/wwdc2020/10611/?time=848)）地理坐标不一定来自数据库。用户点击屏幕后，App 可以从 ARKit 世界坐标反查地理位置，再保存为新的位置锚点。
...
（[20:32](https://developer.apple.com/videos/play/wwdc2020/10611/?time=1232)）LiDAR 设备上，ARKit 4 暴露了 `sceneDepth` frame semantic。启用后，每个 `ARFrame` 都可以读取 `ARDepthData`，其中包含 `depthMap` 和 `confidenceMap`。
...
（[21:12](https://developer.apple.com/videos/play/wwdc2020/10611/?time=1272)）如果 App 已经使用 `personSegmentationWithDepth`，支持 `sceneDepth` 的设备会自动提供场景深度，Session 说明这不会带来额外功耗。
...
（[25:41](https://developer.apple.com/videos/play/wwdc2020/10611/?time=1541)）ARKit 4 推荐用 raycasting 做物体放置。LiDAR 设备上，raycasting 会利用 scene depth 或 scene geometry，在白墙这类缺少特征的表面也能更快给出结果。
