...
The access method of tvOS reuses the Intent extension idea in iOS. The core entry point is still `resolveMediaItems`, which receives `INPlayMediaIntent` to find specific media items from natural language requests.```swift
...
Users on tvOS usually only interact with one foreground App at a time, so it is recommended that the session returns `.continueInApp` in `handle` to let the App start in the foreground mode and start playing.```swift
...
The old practice only returned a media item. Siri can only play back this result.```swift
...
New in 2020, the Plural API can return multiple media items. The first item will be played, with subsequent items shown as alternatives.```swift
...
The sample App ControlAudio changes focus on `resolveMediaItems`. It first takes out `mediaSearch` from the intent, and then hands the search results to the plural resolution API in one go.```swift
func resolveMediaItems(for intent: INPlayMediaIntent, with completion: @escaping ([INPlayMediaMediaItemResolutionResult]) -> Void) {
...
There is no need for a new callback model after the user clicks on an alternative. Siri will set the selected media item into the `INPlayMediaIntent` passed to `handle`, and the App will continue to process it according to the existing playback logic.```swift
func handle(intent: INPlayMediaIntent, completion: (INPlayMediaIntentResponse) -> Void) {
...
If you continue to use extensions, app prewarming can make the app start earlier. Preheating logic should be placed in `application(_:didFinishLaunchingWithOptions:)`, such as preparing credentials or players.```swift
