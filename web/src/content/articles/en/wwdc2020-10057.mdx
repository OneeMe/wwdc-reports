...
([04:48](https://developer.apple.com/videos/play/wwdc2020/10057/?time=288)) Application-level metrics are requested using the App ID. The return value is structured JSON, which is mainly divided into two parts: `productData` and `insights`.
...
([11:19](https://developer.apple.com/videos/play/wwdc2020/10057/?time=679)) The demonstration first uses the App Store Connect API key to generate an access token, and then uses the token to request `perfPowerMetrics`. The official code snippet shows two key headers using `curl`.
...
([08:19](https://developer.apple.com/videos/play/wwdc2020/10057/?time=499)) After discovering the regression, the next step is to get diagnostic signatures from the build dimension. session explicitly states that disk writes diagnostic signatures can be accessed through Xcode and API in 2020.
...
([09:42](https://developer.apple.com/videos/play/wwdc2020/10057/?time=582)) After getting the signature ID, you can request the corresponding diagnostic logs. It contains anonymous diagnostic details from a single device.
