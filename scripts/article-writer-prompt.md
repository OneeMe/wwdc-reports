# WWDC Session 文章生成 Agent Prompt
...
每段讲清楚一个事实，段与段之间用逻辑连接（因果、转折、递进），不要用空泛的过渡句。用具体场景，不要用抽象概念。要有"代入感"，让读者感受到"啊我以前确实被这个问题困扰过"。

如果 session 涉及多个独立主题（如 Platforms State of the Union），用二级标题（`###`）分块讲每个主题的故事。
...
如果 session 是 overview 性质（如 Platforms State of the Union），不用贴完整代码，而是：
- 聚焦 3-5 个最值得开发者关注的亮点
- 每个亮点给出关键 API/概念和简要使用方式
- 标注对应的视频时间戳 `（[MM:SS](视频链接?time=秒数)）`
...
3. **代码驱动**：每个技术亮点必须有至少一个代码示例或关键 API 展示。代码变量名用真实名称（`origamiModel`、`workoutZones`），不要用 `foo`、`bar`。
...
```bash
# session 元数据
node scripts/query.mjs show-session --year {YYYY} --code {code}

# 代码片段
node scripts/query.mjs code --year {YYYY} --code {code}

# Resources
node scripts/query.mjs resources --year {YYYY} --code {code}

# 逐字稿（用于找时间戳和事实依据）
node scripts/query.mjs transcript --year {YYYY} --code {code} --limit 50
```
